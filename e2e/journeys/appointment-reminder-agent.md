...
Test suite for the **Appointment reminder** template card.
...
| First message | Hi {{customerName}}, I'm calling to remind you about your appointment on {{appointmentDate}}... |
| Silence timeout | 10 seconds |
| Max retries | 4 |
| Max call duration | 1800 seconds |
| Barge-in | enabled |
...
| `02-behaviour-tab.spec.ts` | {{appointmentDate}}, silence, barge-in |
...
```bash
npm run auth:save
npm run test:appointment-reminder
npm run test:appointment-reminder-lifecycle
```
