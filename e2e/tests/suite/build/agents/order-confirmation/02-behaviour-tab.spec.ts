...
test.describe("BUILD › Agents › Order confirmation — Behaviour tab @journey @order-confirmation", () => {
  ...
  test("TC-AG-OC-021 @high @positive — First message pre-filled with order confirmation script", async ({
    ...
    if (value.length > 0) {
      expect(value).toMatch(
        ORDER_CONFIRMATION_TEMPLATE.expectedFirstMessageSnippet,
      );
    }
    await firstMessage.fill(ORDER_CONFIRMATION_TEMPLATE.defaultFirstMessage);
    await expect(firstMessage).toHaveValue(
      ORDER_CONFIRMATION_TEMPLATE.defaultFirstMessage,
    );
  });
