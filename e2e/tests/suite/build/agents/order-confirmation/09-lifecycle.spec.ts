...
import {
  createOrderConfirmationAgent,
  openAgentEdit,
} from "../../../../../helpers/order-confirmation.helper";
import { ORDER_CONFIRMATION_TEMPLATE } from "../../../../../data/order-confirmation-template";
import { uniqueName } from "../../../../../utils/test-data";

/**
 * Order confirmation lifecycle (serial):
 * Create → View → Edit → Clone → Delete
 */
test.describe("BUILD › Agents › Order confirmation — Full lifecycle @journey @order-confirmation @serial", () => {
  ...
  test("TC-AG-OC-080 @high @positive — Create order confirmation agent", async ({
    ...
    agentId = await createOrderConfirmationAgent(page, {
      name: agentName,
