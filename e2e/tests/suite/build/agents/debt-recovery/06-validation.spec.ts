...
/**
 * Debt recovery lifecycle edge cases (require a persisted agent).
 * Form validation edges are in ../templates/edge-cases.spec.ts (all cards).
 */
test.describe("BUILD › Agents › Debt recovery — Lifecycle edge cases @negative @debt-recovery @edge", () => {
  test("TC-AG-DR-N114 @medium — Delete cancelled keeps agent in list", async ({
    page,
  }) => {
    const name = uniqueName("DeleteCancel");
    await createDebtRecoveryAgent(page, {
      name,
      language: "en",
