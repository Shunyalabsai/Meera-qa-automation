...
test.describe("BUILD › Agents › Customer support — Behaviour tab @journey @customer-support", () => {
  ...
  test("TC-AG-CS-021 @high @positive — First message pre-filled with support greeting", async ({
    ...
    if (value.length > 0) {
      expect(value).toMatch(
        CUSTOMER_SUPPORT_TEMPLATE.expectedFirstMessageSnippet,
      );
    }
    await firstMessage.fill(CUSTOMER_SUPPORT_TEMPLATE.defaultFirstMessage);
    await expect(firstMessage).toHaveValue(
      CUSTOMER_SUPPORT_TEMPLATE.defaultFirstMessage,
    );
  });
