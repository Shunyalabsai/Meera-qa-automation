...
import { createAppointmentReminderRescheduleAgent } from "../../../../../helpers/appointment-reminder-reschedule.helper";
import { AgentsListPage } from "../../../../../pages/agents-list.page";
import { uniqueName } from "../../../../../utils/test-data";

test.describe("BUILD › Agents › Appointment Reminder & Reschedule — Lifecycle edge cases @negative @appointment-reminder-reschedule @edge", () => {
  test("TC-AG-AR-N114 @medium — Delete cancelled keeps agent in list", async ({
    page,
  }) => {
    const name = uniqueName("AR_DeleteCancel");
    await createAppointmentReminderRescheduleAgent(page, {
      name,
      firstMessage:
